...
public:
    ...
    MODAPI_NDAPI ::CompoundTag& getAndModifyVanillaNetworkTagInfo(std::string_view itemName);

public:
    MODAPI_NDAPI CustomItemRegistry& _modifyItem(std::function<void(CustomItemRegistry&)>&&);

    MODAPI_NDAPI CustomItemRegistry& _registerItem(std::function<std::unique_ptr<Item>()>&&);
};
